<div class="card p-3 my-3" style="background-color: #f4f5f7;">
    <div class="row">
        <div class="col-xl-4 my-2">
            ...
            <select class="form-select form-select-lg @error('employee_id') is-invalid @enderror" name="employee_id" id="employee_id">
                ...
                @foreach ( $employees as $id => $employee)
                    <option value="{{$id}}" @selected($id==old('employee_id',$takeleave->employee_id))>{{$employee}}</option>
                @endforeach
            </select>
            @error('employee_id')
        ...
        <div class="col-xl-4 my-2">
            ...
            <input class="form-control form-control-lg @error('duration') is-invalid @enderror" value="{{old('duration',$takeleave->duration)}}" type="number" placeholder="duration . . ." name="duration" id="duration">
            @error('duration')
                <div class="invalid-feedback">
                    {{$message}}
                </div>
        ...
        <div class="col-xl-4 my-2">
            ...
            <input class="form-control form-control-lg @error('start_date') is-invalid @enderror" value="{{old('start_date',$takeleave->start_date)}}" type="date"  name="start_date" id="start_date">
            @error('start_date')
                <div class="invalid-feedback">
                    {{$message}}
                </div>
        ...
        <div class="col-xl-8 my-2">
            ...
            <textarea  rows="4" class="form-control form-control-lg @error('reason') is-invalid @enderror" name="reason" id="reason" placeholder="reason . . .">{{old('reason',$takeleave->reason)}}</textarea>
            @error('reason')
                <div class="invalid-feedback">
                    {{$message}}
                </div>
